...
The key insight: if we \textbf{store ranks explicitly}, the implementation becomes straightforward and achieves optimal complexity (matching dedicated implementations like DCE).

\paragraph{Extended API.}
Add to the configuration:
\begin{itemize}
  \item $\mathsf{stepInv}(x) = \{ y \mid x \in \mathsf{step}(\{y\}) \}$ --- inverse of step (predecessors)
\end{itemize}
Add to the state:
\begin{itemize}
  \item $\mathsf{rank} : A \to \mathbb{N}$ --- BFS distance from base for each element
\end{itemize}

\paragraph{Expansion Algorithm (BFS).}
...
\textbf{Complexity:} $O(|\text{new elements}| + |\text{edges from new elements}|)$

\paragraph{Contraction Algorithm (Worklist Cascade).}
...
\textbf{Complexity:} $O(|\text{dying nodes}| + |\text{edges to dying nodes}|)$

\paragraph{Why This Is Optimal for DCE.}
For DCE with graph $G = (V, E)$:
\begin{itemize}
  \item $\mathsf{stepFwd}(u) = \text{successors}(u)$ --- $O(\text{out-degree})$
  \item $\mathsf{stepInv}(v) = \text{predecessors}(v)$ --- $O(\text{in-degree})$
  \item $\mathsf{rank}[y] < \mathsf{rank}[x]$ --- $O(1)$ integer comparison
\end{itemize}
This matches the complexity of dedicated graph reachability algorithms.
The rank-based API \emph{generalizes} this to any decomposed fixpoint operator.

\paragraph{Storage Cost.}
One integer (rank) per element. Trivial overhead.

\begin{remark}[Gap Between Proofs and Algorithms]
\textbf{What Lean proves} (specification level):
\begin{itemize}
  \item Semi-naive iteration, when stable, equals $\mathsf{lfp}(F')$
  \item Well-founded cascade, when stable, equals $\mathsf{lfp}(F')$
  \item Soundness of intermediate states
\end{itemize}

\textbf{What the pseudo-code adds} (implementation level):
\begin{itemize}
  \item Concrete worklist/BFS structure
  \item Explicit rank storage and comparison
  \item Iteration order and data structure choices
\end{itemize}

\textbf{Gaps not formally proven:}
\begin{enumerate}
  \item \textbf{Refinement:} The pseudo-code correctly implements the specification.
        \emph{Status:} Visually obvious---each loop iteration corresponds to one spec step.
  \item \textbf{Termination:} The algorithms terminate for finite domains.
        \emph{Status:} Clear for finite sets (each iteration changes a finite set monotonically).
  \item \textbf{stepInv correctness:} The user-provided inverse is correct.
        \emph{Status:} Assumed; for DCE this is just the predecessor relation.
\end{enumerate}

These gaps are ``obvious'' in the sense that closing them would be mechanical.
A good engineer can implement the pseudo-code with confidence.
\end{remark}
...
All definitions and theorems are formalized in Lean with \textbf{all proofs complete} (no \texttt{sorry}).%
\footnote{See \texttt{lean-formalisation/IncrementalFixpoint.lean}.}

\textbf{Proven theorems:}
\begin{itemize}
  \item Well-founded contraction correctness
  \item Semi-naive expansion correctness (including the key lemma that $\mathsf{step}(\mathsf{current}) \subseteq \mathsf{current}$ when stable, using additivity)
  \item Fixpoint monotonicity under expansion/contraction
  \item All helper lemmas for iteration, ranking, and cascade
\end{itemize}
