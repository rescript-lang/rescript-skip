...
\begin{enumerate}
\item \textbf{stepInv correctness:} $y \in \mathsf{stepInv}(x) \Leftrightarrow x \in \mathsf{stepFwd}(y)$
\item \textbf{Element-wise} (automatic): $\mathsf{step}$ decomposes per-element via $\mathsf{stepFwd}$
...
\mathsf{stepInv}(v) &= \{ u \mid (u, v) \in \mathsf{edges} \} \quad \text{(predecessors)}
\end{align*}
\end{example}
