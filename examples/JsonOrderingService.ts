// Service to exercise Json keys of many shapes and the runtime's Json ordering.
import {
...
// Collections:
// - allKeys: a single collection containing keys of many Json shapes.
// - leftKeys/rightKeys: two collections whose merge exercises merge semantics.
type Collections = {
  allKeys: EagerCollection<Json, string>;
  leftKeys: EagerCollection<Json, string>;
  rightKeys: EagerCollection<Json, string>;
};

// Identity resource: exposes allKeys so we can observe its key order.
class AllKeysResource implements Resource<Collections> {
  constructor(_params: unknown) {}
  instantiate(collections: Collections): EagerCollection<Json, string> {
...
class SliceResource implements Resource<Collections> {
  constructor(_params: unknown) {}
  instantiate(collections: Collections): EagerCollection<Json, string> {
...
class TakeResource implements Resource<Collections> {
  constructor(_params: unknown) {}
  instantiate(collections: Collections): EagerCollection<Json, string> {
...
class SlicesResource implements Resource<Collections> {
  constructor(_params: unknown) {}
  instantiate(collections: Collections): EagerCollection<Json, string> {
    return collections.allKeys.slices(
      [false, true], // booleans
      ["", "zzzz"], // strings
    );
  }
...
class MergedKeysResource implements Resource<Collections> {
  constructor(_params: unknown) {}
  instantiate(collections: Collections): EagerCollection<Json, string> {
...
const label = (kind: string, desc: string): string => `${kind}:${desc}`;
