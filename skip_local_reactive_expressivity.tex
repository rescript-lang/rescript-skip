...
We ask: \emph{what is missing from Skip to match the expressivity of
...
However, \emph{anti-joins} (``entries with no matching partner'') are
absent from the 48 core examples---not because they are rare, but because
Skip cannot express them. Two additional anti-join examples (unacknowledged
alerts, orphan detection) illustrate the gap.
Useful reactive patterns like orphan detection (orders with no customer),
unacknowledged-alert dashboards, or unassigned-ticket queues all require
filtering based on absence of keys, which Skip lacks.

We identify an anti-join combinator, \textsf{filterNotMatchingOn}, as the missing operator,
describe how to implement it, and prove that adding it achieves
\emph{expressive equivalence} with RA: the extended combinator algebra
can express exactly the RA queries (selection, projection, union,
...
However, \emph{anti-joins} (``entries with no matching partner'') are
absent from all 48 core examples; two supplementary anti-join examples illustrate
the missing pattern.
...
\end{itemize}
These cannot be implemented in Skip: there is no set-difference operator,
no anti-join primitive, and no way to filter one collection based on
the absence of keys in another.

\paragraph{Contributions.}
We identify an anti-join combinator, $\mathsf{filterNotMatchingOn}$, as the single missing operator,
describe how to implement it in Skip (Section~\ref{sec:comb-fnm}),
and prove that adding it achieves expressive equivalence with relational
...
\paragraph{Anti-joins appear inexpressible.}
While joins are expressible via map-with-lookup, \emph{anti-joins}
(``entries of $R_1$ with no matching key in $R_2$'') do not appear
to be expressible with Skip's current bindings.

Inspecting the Skip API, the \texttt{EagerCollection} type provides:
\begin{itemize}[itemsep=0.2em]
  \item \texttt{map}, \texttt{reduce}, \texttt{mapReduce} --- transformation and aggregation
  \item \texttt{slice}, \texttt{slices}, \texttt{take} --- filtering by key range or count
  \item \texttt{merge} --- union of collections
\end{itemize}
There is no set-difference operator, no anti-join primitive, and no
way to filter one collection based on the \emph{absence} of keys in another.
The \texttt{LazyCompute} mechanism could compute an anti-join on demand,
but produces a lazy collection, not an eager reactive one.

The 48 core examples contain no ``NOT IN'' style queries,
consistent with this limitation; two anti-join examples
(unacknowledged alerts, orphan detection) were added separately to
demonstrate the gap.
The $\mathsf{filterNotMatchingOn}$ operator defined in
Section~\ref{sec:comb-fnm} fills this gap with clear, formally
specified semantics.
