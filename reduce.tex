...
\begin{lemma}[Cancelling a Multiset of Removals]\label{lem:fold-ominus-cancels}
Let $R = (\iota, \oplus, \ominus)$ be a well-formed reducer.
For any finite multiset $M$ and multiset $D \subseteq M$:
\[
\mathsf{fold}_\ominus\big(\mathsf{fold}_\oplus(\iota, M), D\big) = \mathsf{fold}_\oplus(\iota, M \setminus D)
\]
\end{lemma}

\begin{proof}
Enumerate $D$ as a sequence $[v_1,\dots,v_n]$.
By well-formedness, $(a \oplus v_i) \ominus v_i = a$ for all reachable $a$.
Pairwise commutativity of $\oplus$ and $\ominus$ lets us reorder additions and removals, so we can swap each $(\oplus v_i)$ next to its $(\ominus v_i)$ and cancel it, leaving precisely the fold over $M \setminus D$.
\end{proof}
...
By Lemma~\ref{lem:fold-ominus-cancels} with $M = M_0 \uplus \Delta^-(k)$ and $D = \Delta^-(k)$:
\[
\mathsf{fold}_\ominus(a, \Delta^-(k)) = \mathsf{fold}_\oplus(\iota, (M_0 \uplus \Delta^-(k)) \setminus \Delta^-(k)) = \mathsf{fold}_\oplus(\iota, M_0) = a_0
\]
