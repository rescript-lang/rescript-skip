...
\emph{Apache Flink} (Table API) allows user-defined aggregates with \texttt{accumulate} and \texttt{retract} methods, but these are lifecycle methods on a stateful object tied to Flink's query planner and retraction-message algebra.
\emph{Apache Kafka Streams} provides \texttt{Aggregator} and \texttt{Subtractor} interfaces for KTable aggregations, but these are callbacks passed to \texttt{aggregate()} and tightly bound to KTable's update mechanism.
\emph{Esper CEP} supports \texttt{enter}/\texttt{leave} callbacks, but only within sliding-window contexts.
In all cases, the add/remove logic is configuration for a specific operator bound to engine internals, not a reusable, portable combinator.
...
\emph{Functional reactive programming} libraries (Fran, Yampa, Reactive Banana~\cite{frp}) and UI-oriented frameworks (React, Vue, Svelte, Solid) expose declarative combinators like map, filter, and scan/foldp, hiding infrastructure concerns like event routing and cache invalidation.
However, state updates are expressed as opaque folds; the runtime does not know or exploit user-defined inverses.
Developers work declaratively, but without access to the algebraic structure needed for efficient incremental updates with removals.
...
Skip's design occupies a unique position: it hides infrastructure complexity while exposing algebraic structure.
...
This design aligns with a broader trend toward ``event-hidden architectures'' seen in modern distributed systems.
